...
TW_CV_model <- function(expression_RDS, geno_file, gene_annot_RDS, snp_annot_RDS, n_k_folds, alpha, out_dir, tis, chrom, snpset, window, pos_mod_100) {
  ...
  # Shards gene annotation.
  gene_annot <- subset(gene_annot, (gene_annot$chr == chrom) & (gene_annot$start %% 100 == as.numeric(pos_mod_100)))
  ...
  log_df <- data.frame(chrom, n_genes, seed, alpha, pos_mod_100)
  colnames(log_df) <- c('chr', 'n_genes', 'seed_for_cv', 'alpha', 'pos_mod_100')
  write.table(log_df, file = out_dir %&% tis %&% '_chr' %&% chrom %&% 'pos_mod_100_' %&% pos_mod_100 %&%'_elasticNet_model_log.txt', quote = FALSE, row.names = FALSE, sep = "\t")
  set.seed(seed)
  ...
  workingbest <- out_dir %&% "working_TW_" %&% tis %&% "_exp_" %&% n_k_folds %&% "-foldCV_elasticNet_alpha" %&% alpha %&% "_" %&% snpset %&% "_chr" %&% chrom %&% 'pos_mod_100_' %&% pos_mod_100 %&% ".txt"
  write(resultscol, file = workingbest, ncolumns = 9, sep = "\t")

  weightcol <- c("gene","rsid","ref","alt","beta","alpha")
  workingweight <- out_dir %&% "TW_" %&% tis %&% "_elasticNet_alpha" %&% alpha %&% "_" %&% snpset %&% "_weights_chr" %&% chrom %&% 'pos_mod_100_' %&% pos_mod_100 %&% ".txt"
  write(weightcol, file = workingweight, ncol = 6, sep = "\t")

  covariance_out <- out_dir %&% tis %&% '_chr' %&% chrom %&% 'pos_mod_100_' %&% pos_mod_100 %&% '_snpset_' %&% snpset %&% '_alpha_' %&% alpha %&% "_covariances.txt"
  ...
  write.table(resultsarray,file=out_dir %&% "TW_" %&% tis %&% "_chr" %&% chrom %&% 'pos_mod_100_' %&% pos_mod_100 %&% "_exp_" %&% n_k_folds %&% "-foldCV_elasticNet_alpha" %&% alpha %&% "_" %&% snpset %&% ".txt",quote=F,row.names=F,sep="\t")
}
